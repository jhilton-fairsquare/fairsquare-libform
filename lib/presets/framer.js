...
//   required. Field keys are camelCase: fullName | firstName/lastName,
//   email, phone, businessName, zipCode, salesDistributionTier, consent.
//
...
// or plugins, but never shipped on the wire. firstName/lastName are
// combined into `fullName` (the API parses fullName internally — see Field
// Reference). salesDistributionTier is the form key for the revenue
// dropdown; tierMapPlugin derives the spec-documented `annualRevenueRange`
// from it, then the key itself is dropped.
const INTERNAL_FIELDS = new Set(["firstName", "lastName", "salesDistributionTier"]);
...
// Resolve the form-item element for a field. Tries the HTML `name` attribute
// first (the recommended authoring path on every host), then falls back to
// the legacy class wrapper (parity with prod script.js and pages that
// pre-date the name= contract). Returns the matched element or null.
const resolveField = (root, name, klass) => {
  if (!root) return null;
  return (
    root.querySelector(`[name="${name}"]`) ||
    root.querySelector(`.${klass}`) ||
    null
  );
};

const fieldPresent = (root, name, klass) => resolveField(root, name, klass) != null;

const readField = (root, name, klass) => {
...
export function framerForm(config) {
  ...
  // - INTERNAL_FIELDS (firstName, lastName, salesDistributionTier) are read
  //   from the DOM for validators/plugins but never make it to the wire.
  // - tierMapPlugin reads `salesDistributionTier` from the original values
  //   and emits the spec-documented `annualRevenueRange`.
