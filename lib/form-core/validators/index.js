...
// Strict email validator. Inherits prod script's checks
//   - rejects consecutive dots ("..")
//   - rejects duplicate trailing labels ("foo@bar.com.com")
// and fixes prod's bug of only allowing a single domain label
// (so "user@mail.fairsquare.com" now validates correctly).
...
// 2-letter US state code, case-insensitive at the input layer (the API spec
// requires uppercase — the formatter / consumer code can uppercase before
// submit if needed; we accept either here).
...
export const usState = (msg = "Please enter a valid 2-letter state code (e.g., CA).") => async (value) => {
  const s = toStr(value).trim().toUpperCase();
  return US_STATES.has(s) ? ok() : fail(msg);
};

// Expose for testing
// Re-export the enum-based validator so `V.oneOf` is available alongside
// the rest of the validator namespace. Source of truth lives in enums.js
// next to the canonical enum tables.
export { oneOf } from "../enums.js";
