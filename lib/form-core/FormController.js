...
export class FormController {
  ...
  async submit() {
    ...
    // Async enrichment pipeline. Each plugin's `enrich(payload, ctx)` may
    // return the next payload (commonly with new keys merged in) or
    ...
    try {
      ...
      // Plugins are awaited in registration order so an earlier plugin can
      // block a later one (e.g. dataLayerPlugin gating redirectOnUrlPlugin
      // until the GA hit is in flight).
      for (const p of this.plugins) {
        if (typeof p.onSuccess !== "function") continue;
        try { await p.onSuccess(res, this); } catch (e) { this._log("onSuccess plugin error", e); }
      }
      ...
      for (const p of this.plugins) {
        ...
        try { await p.onError(err, this); } catch (e) { this._log("onError plugin error", e); }
      }
