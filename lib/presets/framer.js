...
export function framerForm(config) {
  ...
  // The wrappedValidate `isPresent` guard (above) deliberately treats a missing
  // class wrapper as "this field doesn't exist on this form, skip it." That
  // makes a single preset serve multiple form variants — but it also means a
  // page that's missing every required class submits an empty payload silently.
  // Warn once after mount when a required field's class is nowhere in the form.
    ...
    handle.ready
      .then(() => {
        ...
        if (!has("fullName") && !(has("firstName") && has("lastName"))) {
          missing.push(
            `name — add class ".${cls("fullName")}" to a single full-name input, ` +
            `OR ".${cls("firstName")}" + ".${cls("lastName")}" to two separate inputs`
          );
        }
        ...
        console.warn(
          ...
          `\n\nFix: in Framer, select each input and add the listed class via Properties → Attributes → Class. ` +
